...
from destopPetInteraction.utils.ops import get_edit_distance, read_wav_data
from destopPetInteraction.utils.config import load_config_file, DEFAULT_CONFIG_FILENAME, load_pinyin_dict
from destopPetInteraction.utils.thread import threadsafe_generator
...
class ModelSpeech:
    ...
    def train_model(self, optimizer, data_loader, epochs = 1, save_step = 1, batch_size = 16, last_epoch = 0, call_back=None):
        ...
        '''
        save_filename = os.path.join('destopPetInteraction/save_models', self.speech_model.get_model_name(), self.speech_model.get_model_name())
        ...
        for epoch in range(iter_start, iter_end): # 迭代轮数
            ...
            if epoch % save_step == 0:
                if not os.path.exists('destopPetInteraction/save_models'): # 判断保存模型的目录是否存在
                    os.makedirs('destopPetInteraction/save_models') # 如果不存在，就新建一个，避免之后保存模型的时候炸掉
                if not os.path.exists(os.path.join('destopPetInteraction/save_models',self.speech_model.get_model_name())): # 判断保存模型的目录是否存在
                    os.makedirs(os.path.join('destopPetInteraction/save_models',self.speech_model.get_model_name())) # 如果不存在，就新建一个，避免之后保存模型的时候炸掉
